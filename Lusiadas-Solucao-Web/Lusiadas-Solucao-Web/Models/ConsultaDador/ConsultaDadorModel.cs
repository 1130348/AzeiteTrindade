...
namespace LusiadasSolucaoWeb.Models
{
    public class ConsultaDadorModel : LDFTableModel
    {
        public ConsultaDadorModel()
        {
            ...
            dbParams    = new LDFTableDBParams("BDHLUQLD", "MEDICO", "V_POS_OPER", "*", "", "SALA", null, null);
        ...
        public void LDFTableTreatData()
        {
            ...
            foreach(LDFTableRow item in list_rows)
            {
                ...
                item.rowItems.First(q => q.itemColumnName == "COL1_CLICK").itemValue = "<a data-toggle='modal' data-target='#modalHistoricoDador' data-tdoente='" + tdoente + "' data-doente='" + doente + "' data-nome='" + Generic.GetItemValue(item, "NOME") + "' class='fa fa-lg fa-history consdadorHistorico' title='Histórico Dador'></a>";
                item.rowItems.First(q => q.itemColumnName == "COL2_CLICK").itemValue = "<a data-toggle='modal' data-target='#modalPatologia' data-nome='" + Generic.GetItemValue(item, "NOME") + "' data-patologia='"+Generic.GetItemValue(item, "PAT_ASSOCIADAS")+"' class='fa fa-lg fa-medkit consdadorPatologia' title='Patologia Dador'></a>";
                item.rowItems.First(q => q.itemColumnName == "COL3_CLICK").itemValue = "<a data-toggle='modal' data-target='#modalFormularioDador' data-tdoente='" + tdoente + "' data-doente='" + doente + "' data-nome='" + Generic.GetItemValue(item, "NOME") + "' data-nint='" + Generic.GetItemValue(item, "N_INT") + "' data-nregoper='"+Generic.GetItemValue(item, "N_REG_OPER")+"' class='fa fa-lg fa-heartbeat consdadorFormularioDador' title='Formulário Dador'></a>";
